...
void loop() {
  ...
  // 2) 거리(mm) → 각도(0~180°) 선형 매핑 (18cm→0°, 36cm→180°)
  float angle_deg;
  ...
  // 3) 각도 → 서보 듀티(μs) 변환 후 출력
  int duty = (int)(_DUTY_MIN + ((_DUTY_MAX - _DUTY_MIN) * (angle_deg / 180.0f)));
